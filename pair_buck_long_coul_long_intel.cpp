...
#include "math.h"
#include "pair_buck_long_coul_long_intel.h"
#include "suffix.h"
#include "neighbor.h"
...
PairBuckLongCoulLongIntel::PairBuckLongCoulLongIntel(LAMMPS *lmp) :
  PairBuckLongCoulLong(lmp)
{
...
PairBuckLongCoulLongIntel::~PairBuckLongCoulLongIntel()
{
}

void PairBuckLongCoulLongIntel::compute(int eflag, int vflag)
{
				      ...
				      const ForceConst<flt_t> &fc)
{
  ...
  if (_use_base) {
    PairBuckLongCoulLong::compute(eflag, vflag);
    return;
  }
  #endif
...
void PairBuckLongCoulLongIntel::eval
...
#pragma omp parallel default(none)			\
  ...
  for (int i = iifrom; i < iito; ++i){
	...
	flt_t rexp = exp(-r * rho_invi[typej]);
	if (ORDER6) {
	  if (!ndisptablebits || rsq <= tabinnerdispsq ) {
	    const flt_t grij2 = g2 * rsq;
	    const flt_t a2 = (flt_t)1.0/grij2;
	    const flt_t x2 = a2 * exp(-grij2) * c_energyi[typej].c;
	    forcebuck = r * rexp * c_forcei[typej].buck1 - g8 * x2 * rsq *
	      (((6.0 * a2 + 6.0) * a2 + 3.0) * a2 + 1.0 );
	    if (EFLAG)
	      evdwl = rexp * c_energyi[typej].a - g6 * x2 * ((a2 + 1.0) * a2 + 0.5);

	    if(sbindex){ //Double Check!!
	      const flt_t f = special_lj[sbindex];
	      const flt_t t = (f - 1.0);
	      forcebuck += t * r * rexp * c_forcei[typej].buck1 - 
		         t * r6inv * c_forcei[typej].buck2;
	      if (EFLAG)
		evdwl += t * rexp * c_energyi[typej].a -
		         t * r6inv * c_energyi[typej].c;
	    }
	  }
	  else{
	    float rsq_lookup = rsq;
	    ...
	    forcebuck = r*rexp*c_forcei[typej].buck1 - 
	      (disptable[itable].f + fdisp * disptable[itable].df) * 
	      c_energyi[typej].c;
	    if(EFLAG)
	      evdwl = rexp * c_energyi[typej].a - 
	  ...
	  forcebuck = r * rexp * c_forcei[typej].buck1 - r6inv * c_forcei[typej].buck2;
	  if (EFLAG)
	    evdwl = rexp * c_energyi[typej].a - r6inv * c_energyi[typej].c -
	            c_energyi[typej].offset;
	  if(sbindex){
...
void PairBuckLongCoulLongIntel::init_style()
{

  PairBuckLongCoulLong::init_style();
...
template <class flt_t, class acc_t>
void PairBuckLongCoulLongIntel::pack_force_const
(ForceConst<flt_t> &fc, IntelBuffers<flt_t, acc_t> *buffers){
  ...
  for (int i = 0; i < tp1; i++) {
    for (int j = 0; j < tp1; j++) {
      ...
      fc.c_force[i][j].cut_ljsq = cut_bucksq[i][j];
      fc.c_force[i][j].buck1 = buck1[i][j];
      fc.c_force[i][j].buck2 = buck2[i][j];
      fc.c_energy[i][j].a = buck_a[i][j];
      fc.c_energy[i][j].c = buck_c[i][j];
...
void PairBuckLongCoulLongIntel::ForceConst<flt_t>::set_ntypes
